...
<article class="flex-y flex-o box">
	...
	<details class="flex-y employment-details">
		...
		<ul>
			<li class="job-langs"><?= html_sanitize($D->reqs->must['langs']) ?>.</li>
			<li class="job-frameworks"><?= html_sanitize($D->reqs->must['tools']) ?>.</li>
			<li class="job-experience">
				<?= $D->experience->begin ?> ~ <?= $D->experience->end ?> years.
			</li>
		</ul>
		<div class="job-preferences"></div>
		<ul>
			<?php
			foreach ($D->reqs->opts as $opt) {
